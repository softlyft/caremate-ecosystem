-- Article detail: banner slot below body card.
...
select 'plc_welcome_article_footer', 'camp_house_welcome', 'learn.article_footer'
where exists (select 1 from public.ad_campaigns where id = 'camp_house_welcome' and deleted_at is null)
  and not exists (
    ...
    where campaign_id = 'camp_house_welcome' and slot_id = 'learn.article_footer'
  );
