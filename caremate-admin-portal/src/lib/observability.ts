...
async function reportToSentry(err: unknown, meta?: LogMeta) {
  ...
  try {
    ...
    if (!(globalThis as { __caremateAdminSentryInit?: boolean }).__caremateAdminSentryInit) {
      Sentry.init({
        dsn,
        environment: process.env.SENTRY_ENVIRONMENT ?? process.env.NODE_ENV,
        tracesSampleRate: 0.05,
      });
      (globalThis as { __caremateAdminSentryInit?: boolean }).__caremateAdminSentryInit = true;
    }
    Sentry.captureException(err instanceof Error ? err : new Error(String(err)), {
      extra: meta,
    });
  } catch {
    // Optional dependency must not break request path.
  }
}
