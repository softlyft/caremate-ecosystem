 ...
 *
 * Usage (from repo root or caremate-admin-portal):
 *   npm run bootstrap:admin -w caremate-admin-portal -- user@example.com admin
 */
...
if (!url || !key || !email) {
  console.error(
    'Usage: npm run bootstrap:admin -w caremate-admin-portal -- <email> [admin|editor|support] [--create --password <secret>]',
  );
  console.error(
    'Requires NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in caremate-admin-portal/.env',
  );
  process.exit(1);
}
  ...
  if (!createFlag || !password) {
    console.error(
      `No Auth user found with email ${email}.\n` +
        ...
        `  npm run bootstrap:admin -w caremate-admin-portal -- ${email} ${role} --create --password '<secret>'`,
    );
    process.exit(1);
  }
