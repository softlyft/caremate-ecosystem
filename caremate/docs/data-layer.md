...
2. Execute inline `CREATE TABLE IF NOT EXISTS` for all tables
3. Export `getDatabase()` → Drizzle instance
4. Export `initializeDatabase()` → called once at boot

**Migrations today:** Hand-written SQL in `client.ts`.  
**Drizzle Kit:** Configured in `drizzle.config.ts` to output to `src/database/migrations/` — no generated files yet. Use `npm run db:generate` when adopting formal migrations.
...
| Method | Description |
|--------|-------------|
| `seedIfEmpty()` | Insert FHIR-mapped seeds; soft-delete legacy `provider-1..4` |
| `findAll()` | All providers (typically distance-ordered in UI) |
| `findById(id)` | Single provider |
