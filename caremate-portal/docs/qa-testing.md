...
1. `caremate-portal/.env.local` is configured
2. Shared Supabase migrations are pushed
3. At least one staff user exists via `bootstrap:admin`
4. `provider-ingestion` is running if you are testing provider upload
...
```bash
npm run lint -w caremate-portal
npm run typecheck -w caremate-portal
npm run test -w caremate-portal
```
