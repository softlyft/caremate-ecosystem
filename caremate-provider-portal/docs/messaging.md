...
3. **Care coordination (group chat)** — patient + provider org + payer org thread (payer Pro `group_chat_enabled`).
...
| Reply | `post_patient_message` (also works for `kind = 'direct'`) — **re-checks** live DM eligibility on every send |
| Pair key | `message_direct_pairs` (`organization_id` or `payer_organization_id`, ordered user pair) |
| Push | `notify-message` with `{ mode: 'direct', messageIds }` |

**Provider chat matrix (same org):**

- Org ↔ patient — allowed (org Messages) — **always free** (no Private Care Team plan required)
- Patient ↔ owner/administrator — allowed (DM; **does not** consume a PCT seat)
- Patient ↔ Private Care Team health practitioner — allowed (DM; requires `private_care_team` + `is_health_practitioner`)
- Practitioner ↔ practitioner — allowed (DM) when both are org members
- Patient ↔ staff who is **not** on Private Care Team (and not owner/admin) — **blocked**
...
**Payer:** patient ↔ owner/admin **or** Support Team member (`support_team = true`). See [Payer plans](./payer-plans.md).
...
## Care coordination (group chat)

| Piece | Detail |
|-------|--------|
| Start | Mobile → `start_care_coordination_from_source` / `start_care_coordination_conversation` |
| Candidates | `list_care_coordination_candidates` — only payers with `group_chat_enabled` |
| Gate | Payer org must have Pro+ entitlement with `group_chat_enabled` (`payer_org_has_group_chat`) |
| Participants | Patient + provider org + payer org |

Free/Basic payer orgs cannot open new care-coordination threads.
