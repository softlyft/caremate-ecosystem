...
5. After verify, set password → creates:
   - Supabase Auth user (`care_portal: true`, `care_org_kind`) — **one org kind only**
   - Owner membership (`provider_org_members` or `payer_org_members`)
   - Profile stub with **`verification_status = verified`**
   - Rejected if the email already has any Care Portal membership or is catalogued under the other org kind
...
Returning staff sign in with email/password. Session is valid if the user has an active membership in **either** `provider_org_members` **or** `payer_org_members` (never both for the same email).
...
**Email exclusivity:** A Care Portal email can belong to a provider **or** a payer organization — never both. Claim completion and SoftLyft catalog writes reject cross-type reuse (`care_portal_claim_email_owned_by` + claim checks).

After sign-in, a server action reads membership tables (and optional `care_active_kind` cookie) and sets the destination. Active org cookies: `provider_active_org`, `payer_active_org`.
