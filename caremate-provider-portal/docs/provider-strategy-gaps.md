...
Source: **CareMate Provider Platform Strategy v1.0** (`caremate-docs/CareMate Provider Platform Strategy.pdf`), compared to `caremate-provider-portal`, mobile provider engagement, catalog ingest, and Supabase. Captured **August 2026**; **updated 31 Aug 2026** after retiring portal Laboratory module in favor of Documents.
...
Core engagement spine is shipped. Portal now has a **module activation** surface and fuller **Appointments**. Lab workflows are handled via **Documents** (PDF sharing) — structured lab orders and extraction/mapping are deferred.
...
| Document Management | Partial | Labs, imaging, referrals, discharge → longitudinal record with consent | Bidirectional document vault + signed URLs; labs share results as PDFs | No consent-gated longitudinal publish; no DocumentReference FHIR export; no structured lab extraction |
...
| Laboratory | Partial | Order → sample → process → validate → report → notify → share | **Documents only** — lab orgs upload/share `lab_result` PDFs; prescriptions shared as documents | Structured orders, validation workflow, FHIR DiagnosticReport, analyzer integrations |
| Pharmacy | Gap | Rx, dispensing, inventory, refills | Catalog type only; Rx shared as documents | Entire pharmacy module (Phase 2) |
...
| Capability | Status | Strategy intent | Today | Gap |
|------------|--------|-----------------|-------|-----|
| Capability-based configuration | Partial | “What services?” → auto-enable modules | Catalog + Settings → Modules (`provider_org_modules`); core modules default ON | Auto-enable from org services; more modules on activate UI |
...
| Phase | Intent | Notes |
|-------|--------|-------|
| **1** | Core + Appointments + Laboratory | Core partial; appointments portal-complete (mobile deferred); lab via Documents (structured workflow deferred) |
...
2. **Document intelligence** — extraction/mapping from lab PDFs and prescriptions (deferred)
