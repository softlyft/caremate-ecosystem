...
if (stillMissing.length) {
  console.error('Still missing', stillMissing.length, 'translations:');
  stillMissing.forEach((s) => console.error(' -', s));
  process.exit(1);
}
if (rows.length !== 155) {
...
for (const row of rows) out.push(`  [${row.map((v) => JSON.stringify(v)).join(', ')}],`);
out.push('];', '');
