...
Responsibilities:
1. Open SQLite database with `expo-sqlite` (**SQLCipher** on iOS/Android)
2. Apply `PRAGMA key` from a Keychain/Keystore-backed raw key (`encryption-key.ts`) **before** any other SQL
3. Delete legacy plaintext `caremate.db` once (signed-in users rehydrate via sync)
4. Apply PRAGMAs, baseline legacy installs if needed, then run Drizzle migrations
5. Export `getDatabase()` → Drizzle instance
6. Export `initializeDatabase()` → called once at boot

**Encrypted file:** `caremate.secure.db`  
