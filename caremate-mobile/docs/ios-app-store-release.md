...
Uses GitHub Environment **`prod`**. Signing secrets are the same as TestFlight — see [How to create signing secrets](./ios-testflight-release.md#how-to-create-signing-secrets).
...
### Optional variable (repo or `prod` environment)
...
| Wrong backend in build | Check **`prod`** environment `EXPO_PUBLIC_*` secrets |
