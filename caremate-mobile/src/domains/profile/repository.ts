...
function mapProfile(row: typeof profiles.$inferSelect): Profile {
  return {
    ...
    maritalStatus: (row.maritalStatus as Profile['maritalStatus']) ?? null,
    isHealthPractitioner: Boolean(row.isHealthPractitioner),
...
function profileColumns(profile: Profile) {
  return {
    ...
    isHealthPractitioner: profile.isHealthPractitioner,
...
class ProfileRepository extends BaseRepository {
  ...
  async save(userId: string, input: Partial<Profile>): Promise<Profile> {
    ...
    const profile: Profile = {
      ...
      isHealthPractitioner: input.isHealthPractitioner ?? false,
    ...
    for (const row of data) {
      ...
      await db
        ...
        .values({
          ...
          isHealthPractitioner: Boolean(row.is_health_practitioner),
      ...
      if (keepLocalName && existingLocal) {
        await this.queueSync({
          ...
          payload: {
            ...
            gender: (row.gender as Profile['gender']) ?? existingLocal.gender,
            addressLine: row.address_line ?? existingLocal.addressLine,
            city: row.city ?? existingLocal.city,
            postalCode: row.postal_code ?? existingLocal.postalCode,
            nationalId: row.national_id ?? existingLocal.nationalId,
            maritalStatus:
              (row.marital_status as Profile['maritalStatus']) ?? existingLocal.maritalStatus,
            isHealthPractitioner: Boolean(
              row.is_health_practitioner ?? existingLocal.isHealthPractitioner,
            ),
  ...
  private async reconcileDuplicateLocalRows(userId: string, canonicalId: string): Promise<void> {
    ...
    for (const dupe of dupes) {
      const next: Profile = {
        ...
        gender: fill(merged.gender, dupe.gender as Profile['gender']),
        addressLine: fill(merged.addressLine, dupe.addressLine),
        city: fill(merged.city, dupe.city),
        postalCode: fill(merged.postalCode, dupe.postalCode),
        nationalId: fill(merged.nationalId, dupe.nationalId),
        maritalStatus: fill(merged.maritalStatus, dupe.maritalStatus as Profile['maritalStatus']),
        isHealthPractitioner: merged.isHealthPractitioner || Boolean(dupe.isHealthPractitioner),
