...
class ProviderRepository extends BaseRepository {
  ...
  async syncToRemote(entityId: string, operation: string, payload: unknown): Promise<void> {
    ...
    const { error: stubError } = await supabase.rpc('ensure_provider_catalog_stub', {
      ...
      p_address: provider.address ?? undefined,
      p_phone: provider.phone ?? undefined,
      p_email: provider.email ?? undefined,
      p_latitude: provider.latitude ?? undefined,
      p_longitude: provider.longitude ?? undefined,
      p_distance_km: provider.distanceKm ?? undefined,
